...
### 9. Medium [done] — Already validated history was repeatedly revalidated from genesis

**Locations:** backup `internal/backup/find.go`, `internal/backup/incident.go:131–141`, `internal/backup/commit.go:138–150`, `internal/repository/history_resolve.go`, `internal/repository/history_validate.go`.

The former `resolveHistoryRevision` received a validated `History`, but any spelling other than literal `HEAD` created a new validator without its ancestor cache, fully validated history, then asked the original history whether that commit belonged to it. `auditChain` passed an exact commit ID through this path for every mirror merely to obtain the repository UUID. Resume also constructed an uncached validator for the recorded local commit.

This repeated parsing, Git subprocess work and cumulative-catalog transition validation—not just hash comparisons. As history/catalogs grew, per-mirror local validation repeated historical catalog work already accepted before the audit. Using an exact externally recorded revision also took a materially more expensive path than `HEAD` for the same commit.

**Resolution:** `History.ResolveRevision` retains the original repository/limits, resolves each named expression once, proves membership using the accepted file-backed IDs, then reparses and validates the selected canonical state. It checks the expected parent and immutable FORMAT, preserves tip transition classification against the pinned blob IDs, and validates only the immediate edge when classifying an older ordinary/repair transition. Empty/`HEAD` retains the already pinned tip. Mirror-chain audits use the immutable genesis identity without Git reads, and recorded-commit resumption uses the existing validated-ancestor cache. New/untrusted edges, cache fallback and anchor checks remain unchanged; no new persistent cache, format or provenance mechanism was added.

**Validation:** a failing actual-Git command-trace regression reproduced all three redundant paths before the fix. On its four-commit fixture, named-tip selection dropped from four canonical-tree loads to one; historical selection uses at most two; a tip-chain audit dropped from four to zero; the isolated resume-validation interval dropped from six to three and no longer loads commits before its cached base. Assertions count actual commands, not elapsed time. Additional tests preserve ordinary/repair classification and historical mappings, reject unaccepted commits before state reads, reject malformed/noncommit revisions and selected corrupt objects (including behind a valid tip cache), preserve pinned-tip and selected-state lifetime semantics, and prove selection does not rewrite the ancestor cache. Focused tests and full `make check` passed, including all required linters, coverage, race and vet. No Docker or live cloud contract was run for this local validation change.

### 10. Medium [done] — The Git transport embedded infrastructure convergence and its dependency graph

**Locations:** git-remote-aws `main.go`, `aws.go`, `go.mod`.

The helper imported the full provisioning library for client/config helpers and `ensure=y`. Its compiled dependency graph included 25 AWS service packages, including EC2, IAM, Lambda, ECS, Route53 and Organizations, although its storage protocol uses S3 and DynamoDB. The currently pinned go-dynamolock had already removed its own libaws dependency; that portion of the original finding was obsolete.

Every helper invocation also probed bucket/table setup before handling the Git protocol. Any discovery error was classified as “did not exist”; with `ensure=y`, it entered convergence even for permission, network or other non-absence failures. Without it, the diagnostic still falsely claimed absence.

**Decision and resolution:** retain `ensure=y`, remove libaws, and introduce no S3/DynamoDB data migration. Existing deployments have already completed the earlier lease-format migration; this change does not reopen or repeat it. Helper commit `8b5512f` creates its concrete S3/DynamoDB/STS clients directly from one SDK shared-config load, preserves profiles/credentials/regions/endpoints and the five-attempt request policy, and removes libaws plus its unused graph. Actual compiled AWS service dependencies drop from 25 to six, including the SDK's credential-provider services. Existing resource configuration, bucket/prefix keys, DynamoDB envelopes, encryption and lease/CAS behavior remain unchanged.

`ensure=y` now creates only confirmed-missing resources with the prior private-bucket and on-demand id-keyed-table defaults. Other discovery errors preserve their causes and cannot authorize convergence. New-bucket setup retains the security defaults and existing setup tag, but never converges an already-existing bucket or table. Resource setup must be serialized; S3 CreateBucket is not automatically retried because its us-east-1 existing-owned-bucket behavior can reset ACLs. Incomplete new-bucket setup leaves the resource in place, reports failure and requires administrative completion, not deletion/recreation or a new resumability subsystem.

**Validation:** a failing actual-CLI regression first reproduced permission failures being misclassified for both services and both ensure settings. Tests then covered absent/existing resources, regional creation inputs, private/HTTPS/encryption defaults, no configuration or record mutations on existing resources, uncertain CreateBucket and incomplete setup, profile credentials/session tokens/endpoint overrides, retry budgets and cancellation. Checked-in SHA-1 and SHA-256 ciphertext/DynamoDB fixtures were generated with pre-removal helper `3ebf0a3`; the new helper lists, decrypts/imports and extends both histories without migration or rewriting their existing ciphertext. Focused race tests, the full cloud-free coverage selection (75.3%) and `GOTOOLCHAIN=local bash bin/check.sh` passed. The script still emits its pre-existing advisory go-hasdefault/go-hasdefer messages; finding 15 remains separate. No live AWS resource was contacted or changed, no binary was installed, and nothing was pushed.

### 11. Medium [done] — Provisional add required source quiescence that final capture deliberately does not require

**Locations:** backup `internal/filesystem/scan.go` (`scanRegular`), `internal/backup/add.go` (`buildAddPlan`).

Add hashed to unbounded EOF, then rejected any change in size, timestamps, mode or other captured identity. Commit instead read at most the open-time size once and recorded the actual readable bytes, warning about mutation. Thus an actively appended log or modified large file could prevent obtaining any add plan, making the mutation-tolerant commit semantics unavailable. Add could also chase continuing appends rather than bounding its provisional read to the initial size.

**Resolution:** provisional hashing now makes one bounded pass, records the exact hash and byte count, retains open-time mode/mtime, and emits a `file-changed` diagnostic for detectable mutation. Growth beyond the initial size is deferred; truncation records the shorter observation, including an empty file, without omitting the path. No retry, plaintext retention, mtime cache, path-selection change, or canonical-format change was added. No-follow and opened-inode checks, timestamp bounds, visible source-permission skips, and fatal I/O/unsafe-path failures remain in force. The behavior is documented in `NINA.md`.
