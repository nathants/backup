...
A backend is not production-eligible merely because ordinary uploads work. Before it is accepted, run its real cloud/server contract suite with the exact ordinary writer credential and retain an immutable probe object. The suite must prove that new-key conditional creation works and that the credential itself cannot overwrite, delete, copy-overwrite, multipart-overwrite, or alter the immutability configuration protecting an existing key—even when requests deliberately omit `If-None-Match`. Required enforcement is backend-specific:

- `backup-server`: its narrow server protocol itself requires create-only PUT and exposes no destructive API.
- AWS S3: a dedicated writer IAM policy plus bucket policy denies deletion and rejects object creation without `If-None-Match: *`; require ordinary SSE-S3 (`AES256`) for these already recipient-encrypted objects and disallow SSE-KMS and SSE-C. Versioning/Object Lock may add defense in depth.
...
Run whole-root client tests in an isolated client container, with the server in a separate container. Credential-gated contract suites run the same real client/backend contract against AWS S3 and R2 using Admin-supplied accounts; they are excluded from default tests but are mandatory before that backend is production-eligible. Cloud tests use dedicated buckets or prefixes and unique immutable namespaces and never depend on cleanup/deletion.
...
The real backup binary then completed an AWS-only test revision against that bucket: genesis, add/commit, one-mirror success, checksum-only verify, full restore with content/mode/nanosecond-mtime/symlink and independent-inode checks, and metadata recovery with the primary Git remote unavailable at both the latest tip and the explicitly anchored genesis tip. Private credentials, recovery material, exact commits, retained namespaces, and the probe are recorded under `<private-contract-evidence>`. One 4.5 KiB immutable namespace from an earlier failed test-harness assertion has no retained recovery key; it is harmless test garbage and is explicitly recorded there rather than hidden.
...
1. Run the exact configured AWS S3/R2/server backend contracts, including checksum persistence/no-body HEAD verification and destructive negative tests with each ordinary writer credential; retain and recheck the immutable probe objects.
...
8. Restart the production local server against the same data directory and repeat verification/restore.
