...
**Accepted filesystem security boundary:** directly writable local disks do not resist a compromised user with filesystem write access. Native create-only operations prevent accidental replacement by backup, not direct overwrite/deletion by another process. This is an explicit exception to the network backends' enforced ransomware boundary. Use ext4, exclusive control of the store and its ancestors, and disconnect removable storage between operations when practical. One local mirror can satisfy runtime completion; it is not redundant until additional mirrors are synchronized. The Git primary remains mandatory. Read [Filesystem mirrors](docs/filesystem-mirrors.md) before configuring, modifying, or operating this backend.
