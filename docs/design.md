...
2. **One or more object mirrors**:
   - a native `filesystem` backend writes directly to operator-controlled ext4 storage, including removable disks;
   - remote `backup server`, AWS S3, and Cloudflare R2 use the same narrow S3-compatible client protocol;
...
All three network object backends use one ordinary credential per mirror for read, list, and immutable creation. The same profile serves backup, restore, verify, sync, and repair; overwrite, deletion, configuration changes, and credential/permission escalation remain forbidden. Provider/account administration stays separate and off backup clients. A stolen ordinary credential can read historical ciphertext and plaintext completion manifests, but cannot decrypt encrypted content without a recipient secret. Read access is part of the ordinary credential; separate upload-only/read-only roles are not required. A cloud read-only credential remains an optional deployment restriction for an audit/restore-only machine, not another configured role. The backup server exposes one read/list/create credential, not a role system.

**Accepted filesystem security boundary:** directly writable local disks do not resist a compromised user with filesystem write access. Native create-only operations prevent accidental replacement by backup, not direct overwrite/deletion by another process. This is an explicit exception to the network backends' enforced ransomware boundary. Use ext4, exclusive control of the store and its ancestors, and disconnect removable storage between operations when practical. One local mirror can satisfy runtime completion; it is not redundant until additional mirrors are synchronized. The Git primary remains mandatory. Read [Filesystem mirrors](filesystem-mirrors.md) before configuring, modifying, or operating this backend.
...
By default the client reads `$BACKUP_ROOT/.backup-config`, where `BACKUP_ROOT` defaults to `/`. The file must be a bounded regular file that is not group/other writable. Trusted config, custom CA, and staged-file readers use nonblocking no-follow opens and descriptor regular-file checks so a FIFO cannot stall validation. Its raw tab/LF format is:
...
A network mirror row is `mirror name kind storage_url endpoint region profile ca_file`, with fields separated by tabs. A filesystem row uses the same eight-field width: `mirror name filesystem filesystem://STORE_ID - - directory required_mount`. `STORE_ID` is a persisted random 32-lowercase-hex identity, not a pathname. Directory and required mount are canonical absolute, no-symlink local paths; `-` permits an ordinary directory without a mount-point requirement. Use the required-mount binding for removable disks. Filesystem mirrors never load AWS credentials or TLS settings; their local path bindings are not canonical metadata. Use `-` for a provider-default endpoint, an absent credential profile, or system trust roots. Configure one standard AWS shared-credential profile per network mirror. This is a hard cutover: the old nine-field dual-profile rows are rejected; use eight fields. Canonical table shapes and encrypted object formats are unchanged; older binaries reject the new filesystem kind. Local CA paths and credentials remain local; canonical `mirrors.tsv` contains only nonsecret topology. Before any network request, trusted configuration must pin and exactly match the selected canonical mirror's name, kind, bucket, prefix, endpoint, and region. Before filesystem I/O, pin the canonical name/kind/store ID and require the configured directory's marker to match; check the required mount when present. Never create a missing store during backup or recovery.
...
```text
name    kind    storage_url    endpoint    region
```

Names are unique and rows are sorted by name. Once introduced, a name and its kind/bucket/prefix/endpoint/region identity are permanent in history; removing a mirror later is allowed, but rebinding or reusing its name is not—add a new name instead. `kind` is a closed backend identifier: `filesystem`, `backup-server`, `aws-s3`, or `cloudflare-r2`; unknown kinds fail closed. For filesystem mirrors, `storage_url` is exactly `filesystem://` plus the store's 32-hex identity, and endpoint/region are `-`; name/kind/store identity are permanent, while local mount paths can be repinned explicitly. For network mirrors, `storage_url` is an `s3://` URL containing bucket and prefix; endpoint is `-` for the provider default. The permanent bucket/prefix is the repository's object-store namespace: repositories sharing a bucket require unique, non-overlapping prefixes, and credentials plus immutability controls must cover the intended exact prefix. `FORMAT` does not duplicate this routing identity. Secrets and local CA paths are never tracked.
...
After accepting history, resolve named revisions once and prove membership in its file-backed commit list before loading the selected canonical state. Revalidate that state and, for a historical non-genesis commit, its immediate transition—not every accepted ancestor. Empty/`HEAD` selects the already pinned tip. Mirror-chain audits reuse the immutable genesis identity; commit resumptions reuse the existing validated-ancestor cache. Cache anchors are reparsed and checked; newly accepted edges still undergo normal canonical/transition validation. An absent or malformed cache falls back to full history validation. These are local acceleration paths, not another provenance store or a change to canonical metadata.

Diff consumes the validated-ancestor cache read-only, without publishing cache changes. Interrupted alternate-spool cleanup also reuses this cache. Both retain anchor/selected-state validation and full-history fallback; cached history cannot authorize an unrelated plan base.
...
Always exclude `$BACKUP_ROOT/.backup`. Also exclude configured filesystem destination paths beneath the source root with a visible diagnostic; reject a source root inside a store and reject a saved plan containing a newly configured destination before capture/first publication. Bind-mount and hardlink aliases still require explicit operator exclusion; configured-path exclusion is not alias discovery. When root is `/`, additional default exclusions include at least `/dev`, `/proc`, `/run`, `/sys`, and `/tmp`; exclusions are explicit and auditable. Reject any snapshot in which a file or symlink path is an ancestor of another index path. Source `EACCES`/`EPERM` failures during stat, open, read, or symlink resolution omit the affected path with a `permission-denied-skipped` diagnostic naming the path and failed operation. An inaccessible directory prunes its subtree; the add summary counts skipped paths/directories, not unknown descendants. Never change source permissions or elevate privileges automatically. Omitted paths are absent from the new snapshot, not silently carried forward from an earlier revision. Root access, ignore-policy reads, metadata/staging access, I/O errors, and unsafe traversal still fail closed. A zero-entry snapshot requires explicit `--allow-empty`; otherwise fail rather than committing a silent empty snapshot.
...
Git ignore selection happens at add or explicit replan; commit retains the last successful path plan. One streaming `git check-ignore` process per active matching scope avoids per-path subprocesses and a duplicate path catalog. Its NUL-delimited responses are bounded to 64 KiB per field; ignore files are bounded to 16 MiB and Git marker/HEAD/common-directory text to 64 KiB. Missing metadata permits pattern-only matching, but read/permission errors do not. Unreadable ignore files, configuration/index errors in a recognized repository, failed commands, and oversized queries fail the scan rather than silently changing exclusion semantics. Source Git commands retain user ignore configuration but disable hooks, fsmonitor, optional locks, and network transports, and clear inherited repository/pathspec overrides. This is distinct from the metadata Git runner's stricter configuration isolation.

`backup add` is a planning scan, not the content-capture boundary. The last successful add or replan fixes the exact lexical path set eligible for the revision plus provisional kinds, hashes, sizes, modes, mtimes, and symlink targets. A later commit never enumerates the source tree and never includes a path first created after that planning operation. Commit opens each planned path descriptor-relatively beneath `BACKUP_ROOT` without following any symlink component, captures its current supported kind, content, and metadata, and writes only those final observed values to canonical metadata. A planned path changed since add is captured in its current form and produces an informational warning; a disappeared, broken, outside-root, permission-denied, directory, or special path is omitted with a warning. Source permission denials never authorize ignoring plaintext-spool creation, write, sync, or cleanup failures. I/O, unsafe traversal, and other errors remain fatal. If every planned path is omitted, commit fails unless the plan was explicitly created with `add --allow-empty`.

Commit prints every omitted path and reason when observed, including during resumed capture. Only successfully captured changes use the 100-detail warning cap and aggregate remainder; that cap never hides omitted paths. Mandatory diagnostic write failures remain fatal.

Add-time regular-file hashing also makes one pass bounded by the open-time size. The provisional hash and size describe the exact bytes read, while mode and mtime come from the opened descriptor before hashing. Appends beyond the bound are deferred; early EOF after truncation records the shorter observation, including an empty file. Detectable changes emit a `file-changed` diagnostic rather than aborting or omitting the path. Add retains no plaintext and does not retry for a stable file. No-follow traversal, opened-inode validation, timestamp bounds, the source-permission skip policy, and fatal I/O errors still apply.
...
Accumulated repair descriptors remain a separately hashed JSON array, streamed with a 64 KiB per-record/read-ahead bound rather than a whole-array size ceiling or an in-memory descriptor slice. Writer and reader enforce the same record bounds. Bounded external sorting detects duplicate parts/paths and matches the candidate catalogs; upload cursors retain append order, not catalog sort order. Appending or relocating a descriptor writes and fsyncs a new content-addressed file before atomically replacing the control record, retaining the prior usable candidate if publication fails. This does not change the operational-state schema or canonical backup format.

Within one locked runtime, acknowledgement-only saves reuse the current successful repair descriptor/catalog validation, keyed by exact descriptor and candidate-pack references (paths, hashes, sizes), descriptor count, and staged metadata paths used for collision checks. Candidate-state reuse likewise requires exact file references. Loading/restarting or changing relevant inputs requires fresh validation; failures are never cached. Control/progress checks and durable acknowledgement writes remain per-save, while descriptor/payload consumption retains its integrity checks. This is bounded in-process reuse, not a persistent cache or protection against concurrent same-user staging edits.
...
A small private `preparation.json` pins the exact initial `FORMAT` hash and identifies this intentionally unborn state. Before first publication, `add`, `replan`, `diff`, and `reset` work locally without remotes or credentials. Add permits absent trusted configuration; if supplied, it validates it locally to exclude filesystem destinations, without opening mirrors or loading credentials. They use the normal bounded scanner and file-backed path plan against empty catalogs, reject unrelated metadata edits, preserve identity/recipient checks, and never guess that a repository missing its preparation record is fresh. Reset discards the plan, not initialization or configuration. `init` refuses every nonempty existing metadata directory; it never deletes one or resumes publication.

The first `backup commit` requires an add plan, nonempty valid recipient chains, and complete trusted Git/mirror configuration. It checks that mutable metadata still matches the add-time plan. Before writing the destination pin, native Git validates the configured literal branch name; the config parser's bounded lexical grammar alone is not sufficient. Invalid names leave the preparation record, add plan, origin, and HEAD unchanged, so configuration can be corrected and retried or reset. Valid slash-containing names such as `archive/home` are supported by both backup and the current helper; use a helper with that support before first publication. Existing durable pins are never cleared automatically. If the planned `mirrors.tsv` is empty, this first commit fills it from the trusted mirror configuration; a nonempty table must already match exactly. Recipient edits still require another `add`; ignore-only edits may use `replan`. The selected source path set is unchanged even when configuration or source files are created after the last add/replan.
...
Git setup `HEAD`, `config`, exclusion file, and initial directories are fsynced before reporting local initialization or completing first remote binding; ordinary Git object/reference fsync hardening remains in force. Managed initialization also persists the `.backup` entry in its containing directory before Git setup. Missing setup ancestors are created and fsynced individually before their descendants, including a containing-directory barrier when reusing an ancestor after an interrupted attempt. Parent lookup uses the opened directory descriptor. This is localized setup durability, not custom journaling or a universal storage power-loss guarantee.
...
`backup replan` explicitly refreshes an existing add-only plan's selection after ignore edits, without rehashing known regular-file paths. It traverses the current eligible tree using the same backup/Git ignore rules, no-follow traversal, mount diagnostics, permission policy, and filesystem destination exclusions as add. It discovers new paths anywhere eligible and removes disappeared, ignored, or unsupported paths. A regular path with a saved regular-file observation reuses its exact provisional hash, size, mode, and mtime even when changed on disk; this is not an mtime/size trust cache. Newly eligible regular paths without such an observation are hashed once. Symlinks are always resolved afresh. Commit still captures every selected path's current supported content and metadata; use ordinary add to refresh all provisional observations.

Replan preserves the exact saved base and `--allow-empty` choice, fetches/uploads nothing, and accepts only `ignore` edits to tracked configuration. Recipient/topology edits or a changed local HEAD require add. It refuses capture/publication/reset progress. Replacements are private, hashed, fsynced plan generations followed by one atomic transaction-record replacement; pre-publication failure leaves the old plan usable and post-publication interruption resumes from the complete new plan. An optional file-backed observation inventory retains previously selected rows even when subsequently excluded, so undoing an exclusion can reuse them. It may contain former leaves and their descendants from different scans and is never a snapshot or capture input. The inventory is validated only when replan reuses it; an absent or corrupt inventory makes replan fail and recommend a fresh add, but does not block diff, add, reset, commit, or commit resumption. The selected plan and its configuration retain their normal mandatory integrity validation. A disposable on-disk path-hash/offset table bounds lookup memory and checks the actual path before reuse. Existing add-only plans use their own index as the initial inventory; ordinary add discards the inventory and reset/transaction cleanup removes it. Before rebuilding, add and replan reclaim unreferenced plan generations under the repository lock, protecting every generation referenced by the loaded transaction. A save error never triggers deletion based on the unconfirmed replacement; retries reload the authoritative record before cleanup, including after an ambiguous publication barrier. No plaintext is retained. Cost is current directory traversal, hashing newly observed files, and bounded metadata workspace—not rereading known file contents. Replan reports reused/hashed regular-file counts. Diff remains read-only; no reset is needed before add or replan.

`backup diff` compares the last successful add/replan selection with its saved base without materializing candidate files into the Git worktree. It is authoritative for path selection but provisional for content hashes, sizes, kinds, modes, mtimes, and symlink targets; the successful commit ID identifies the exact captured revision.
...
Timeouts, credentials/TLS failures, missing checksum capabilities, and ordinary mirror lag are not proof of corruption. Superseded historical data keys do not quarantine a compatible healthy current mapping, and an unusable metadata representation does not quarantine a mirror when another eligible representation supplies a complete verified chain. Restore, sync, and data-repair reads also record conclusive failures of current data objects. The local incident state contains a counter, mirror quarantine set, and optional forward-repair anchor, not an unbounded per-object incident log. Operational state version 4 rejects older transaction/ledger versions; it does not migrate or discard them. Finish old staged transactions with their original binary before upgrading, retain the old ledger separately, and rebuild the new ledger through verification.
...
A repair candidate is eligible only after fresh verification of its complete candidate data catalog plus its base metadata chain; it must not require a falsely healthy base data catalog. When several relocations are needed before any mirror can be complete, repeated `repair data` calls accumulate parts in the same unpublished repair candidate; retrying an already staged part reuses its durable bytes/key. No partial repair is reported successful. Once a repair has a local commit, resume it with `commit` (or use explicit forward repair if a new incident affects its published tip). `find`, verification, restore, sync, and metadata repair remain available during pending publication; once the transaction's local commit is accepted, read operations validate the local accepted tip without fetching or materializing another tip over the transaction. Metadata repair of the pending tip requires proof that its Git publication occurred.

While a transaction is pending, its exact staged metadata manifest hash and object ID pin the only physical representation eligible to complete that Git edge. Verification, sync, commit finalization, and forward-repair retirement do not substitute another manifest merely because it declares the same commit and has matching object checksums. Before finalizing from a completion-ledger row, commit freshly checks an individual mirror's complete data catalog and metadata chain with that pending pin; a commit ID in the ledger alone does not authorize staging cleanup. An explicit `repair metadata` may replace the pin: it first decrypts/imports and validates its locally rebuilt representation, publishes its parts and manifest, then atomically adopts durable replacement staging and its actual destination acknowledgements into the existing transaction. Old staging remains until transaction cleanup. Interrupted pre-adoption repair leaves the old pin intact and may be retried; post-adoption commit resumes from the new pin. This adds no permanent provenance store, signing keys, object-format change, or restore prerequisite.
...
3. Derive part keys as `metadata/parts/<part_blake2b>/<object_id>` and manifest keys as `metadata/manifests/<tip_commit>/<manifest_blake2b>/<object_id>`; do not store redundant complete keys inside the manifest. The repeated tip segment is a deliberate validated index so anchored `--tip` recovery need not inspect unrelated or attacker-flooded manifests. All keys are create-only (application-enforced on filesystem mirrors, storage-enforced on network mirrors). Upload all bundle parts first and the hash-addressed manifest last; the manifest is the sole completion marker. Cloud verification may fetch these bounded manifests, but never pack bodies or encrypted bundle-part bodies.
...
`backup sync` catches up a missing/lagging mirror from a healthy mirror by creating absent immutable keys and metadata deltas. It never overwrites or deletes. It uses each network mirror's ordinary profile, or local filesystem permissions: reads at the source, and reads plus immutable creates at the destination. An existing destination key is checksum-verified rather than trusted from `412` or existence.

Historical sync still audits/copies the selected revision, even when the ledger records a validated descendant whose current catalog may use relocated keys. It retains that newer completion evidence without treating retention as an error; historical success never clears a current quarantine.
...
Verification always validates canonical metadata and cross-file invariants plus the declared metadata-manifest chain through the selected tip. Required-object verification is backend-specific so checking large cloud mirrors does not transfer their bodies.

**Accepted checksum-only audit limit:** outside the pending transaction's pinned edge, manifest-chain verification checks declared repository/edge relationships and provider checksums, not whether unfamiliar ciphertext actually reconstructs those declarations. An ordinary create credential can add a false but checksum-consistent representation that masks a missing genuine edge. This cannot overwrite or destroy an already healthy immutable chain, and recovery rejects false representations by decrypting/importing them; it can nevertheless produce a false health/completeness claim for an absent or damaged chain. The runtime completion invariant relies on honest initial construction/publication and does not turn checksum-only audits of unfamiliar historical representations into a cryptographic proof of recoverability. Retain external revision anchors and exercise actual metadata recovery, especially after compromise or loss of local state. No permanent provenance database, signing-key role, or extra restore prerequisite is introduced to remove this accepted limitation.

Required-object checksum paths:
...
A mirror counts toward `--minimum-mirrors` only when its declared manifest chain passes the checks above and every currently required pack and bundle-part object is present and agrees with the strongest trustworthy checksum that backend exposes. If a provider exposes neither a trustworthy full-object SHA-256 nor a documented MD5 for the exact upload mode, verification reports that mirror as unverifiable rather than silently falling back to existence/size.

Restore itself performs complete ciphertext, encryption-authentication, tar-member, and plaintext-hash verification for the selected content. `backup verify --full` counts only native filesystem mirrors: it streams and decrypts every catalog pack through the real decompression/tar/plaintext-hash reader and reconstructs the exact selected metadata tip through the recovery importer. It requires an eligible secret source, bounded file-backed metadata workspace, and the same operator-owned resource containment as recovery. Ordinary filesystem verification needs no secret and recomputes all recorded ciphertext hashes and sizes locally. Full decrypt verification without restore applies only to this directly accessible filesystem backend. Every protocol backend—including `backup-server`, AWS S3, and R2—is checksum-only: `backup-server` hashes bytes internally for checksum-enabled HEAD but does not transmit them, just as S3/R2 verification does not download object bodies.
...
The ordinary network client has no effective overwrite/delete capability against existing protected remote objects. Filesystem writes use create-only publication too, but direct disk access is the accepted security exception described above. For data packs, `backup repair` uses the ordinary profiles to read a healthy source and create immutable objects at each destination, then creates a normal fast-forward relocation commit. For metadata bundles it publishes only the alternate physical representation described above and creates no Git-history edge.
...
## Native filesystem mirrors

`backup mirror-init --directory DIRECTORY [--mount MOUNT]` explicitly provisions an existing empty ext4 directory with a random, fsynced `.backup-store` identity marker. It does not initialize source metadata, bind Git, mount a disk, or open network clients. An absent/wrong marker, changed root inode, or missing required mount fails closed. An unavailable disk is not automatically a corruption incident. Directory opens use no-follow traversal and the backend locks the opened store directory exclusively for the command lifetime.

The backend implements the same narrow object-store interface as the network client. Encrypted object keys/bytes, metadata chains, completion ledgers, sync, and immutable repair are unchanged. Creation copies into a private same-filesystem random temp, verifies all hashes and size, fsyncs, publishes with `renameat2(RENAME_NOREPLACE)`, and fsyncs containing entries. Parent barriers include reused directories; a post-publication barrier failure is ambiguous. Audits recompute content hashes and flush file/directory barriers before acknowledging existing bytes. Destination capacity preflights retain the configured space reserve; they do not reserve space. No published object is overwritten or removed. Stale recognized regular temps are cleaned only before writing under the exclusive lock, never on reads.

Listing uses bounded batched traversal, sorted keys, a one-million-matching-key ceiling, and a proportional directory budget; anchored metadata listing prunes unrelated tips. No database or full-tree startup index is required. Configured local mirrors must have distinct identities and non-overlapping directory bindings. The store root and all ancestors require operator control; simultaneous external writers and destination aliases within the source set are unsupported. See the [filesystem setup, verification, and later-cloud-sync runbook](filesystem-mirrors.md). Tests use ordinary temporary directories, including absent-mount rejection, and require no mounted disk.
...
The server acquires an exclusive process lock for its data root before serving. Temporary uploads live in a dedicated `0700` internal directory on the same filesystem, are never addressable/listed as objects, and use random `0600` no-follow files. Failed requests remove their temp; after acquiring the root lock, startup safely removes stale regular temps and fails closed on unexpected types or cleanup errors. Startup fsyncs the data root's containing directory through the opened root descriptor. Every storage-directory open on the create path, including an already-existing entry, fsyncs its parent before permitting publication beneath it: another request's visible mkdir is not evidence that its durability barrier completed. Barrier failures propagate rather than allowing a success acknowledgement.
...
Every checksum-enabled HEAD retains the opened final containing-directory descriptor and fsyncs it after content verification, before acknowledging success. This closes a failed or pending PUT publication barrier; file and ancestor barriers already precede publication. Audit-barrier failures remain generic availability errors, not corruption evidence. Ordinary HEAD and GET do not add this barrier.
...
# After editing only .backup/ignore, avoid rehashing known files:
backup replan --root /data
backup diff --root /data
...
Every CLI invocation, including help and parse failures, also captures its stdout/stderr in `~/.backup-logs/`. Private per-invocation JSON Lines files label UTC time, run ID, allowlisted command name, and stream, with separate start and exit-status records. No arguments or environment are dumped. Text is streamed in bounded chunks, with no whole-command output buffer. Files rotate on the first write after a UTC day change, including for a continuously running server. Each invocation and rollover removes only recognized, owned regular log files last modified more than 14 days ago, excluding hardlinks; it never recursively removes directories or follows log-directory/entry symlinks. The application directory is created as `0700` and files as `0600`; an unsafe existing directory disables disk logging rather than changing its permissions. Retention is age-only, with no byte cap.

Disk logging is best-effort: setup, cleanup, write, and close failures warn on stderr, disable that invocation's disk logging, and never change the operation's result. Progress destinations fail independently: a failed stderr sink stops receiving optional progress while healthy disk logging continues, and a failed disk sink does not stop terminal progress. Mandatory terminal-output failures still propagate regardless of the disk copy; optional progress remains safe with a closed stderr pipe. These logs may contain paths and results and require private operator control. They are not fsynced publication evidence or an external revision-anchor archive. Backup selection is unchanged; use the backup ignore file to exclude this diagnostic directory when it lies beneath the source root and should not be archived.
...
1. Complete `make integration`, the separate [AWS Git-primary gate](git-primary-contract.md) (`make integration-git-remote`), and the exact-production contract runbook above for every configured cloud backend; retain and recheck each immutable probe. The Git-primary gate requires the independent pre-keychain helper artifact and provisions/cleans its own guarded scratch S3/DynamoDB resources, without expanding mirror credentials.
...
8. Accept the exact production `backup-server` deployment as described above when configured. For filesystem mirrors, complete the filesystem runbook's verify/full-verify, restore, and anchored-recovery checks against the actual ext4 destination; temporary-directory tests do not qualify hardware or power-loss durability.
