...
Disk logging is best-effort: setup, cleanup, write, and close failures warn on stderr, disable that invocation's disk logging, and never change the operation's result. Mandatory terminal-output failures still propagate; optional progress remains safe with a closed stderr pipe. These logs may contain paths and results and require private operator control. They are not fsynced publication evidence or an external revision-anchor archive. Backup selection is unchanged; use the backup ignore file to exclude this diagnostic directory when it lies beneath the source root and should not be archived.
